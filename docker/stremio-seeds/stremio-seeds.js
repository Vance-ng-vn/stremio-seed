...
var require_util = __commonJS({
  "node_modules/bencode/lib/util.js"(exports2, module2) {
...
var require_encode = __commonJS({
  "node_modules/bencode/lib/encode.js"(exports2, module2) {
...
var require_decode = __commonJS({
  "node_modules/bencode/lib/decode.js"(exports2, module2) {
...
var require_encoding_length = __commonJS({
  "node_modules/bencode/lib/encoding-length.js"(exports2, module2) {
...
var require_lib = __commonJS({
  "node_modules/bencode/lib/index.js"(exports2, module2) {
...
var require_blob_to_buffer = __commonJS({
  "node_modules/blob-to-buffer/index.js"(exports2, module2) {
...
var require_thirty_two = __commonJS({
  "node_modules/thirty-two/lib/thirty-two/thirty-two.js"(exports2) {
    "use strict";
    ...
    exports2.encode = function(plain) {
...
var require_thirty_two2 = __commonJS({
  "node_modules/thirty-two/lib/thirty-two/index.js"(exports2) {
    var base32 = require_thirty_two();
    exports2.encode = base32.encode;
    exports2.decode = base32.decode;
  }
});

// node_modules/bep53-range/index.js
var require_bep53_range = __commonJS({
  "node_modules/bep53-range/index.js"(exports2, module2) {
...
var require_magnet_uri = __commonJS({
  "node_modules/magnet-uri/index.js"(exports2, module2) {
...
var require_simple_sha1 = __commonJS({
  "node_modules/simple-sha1/index.js"(exports2, module2) {
    var crypto = require("crypto");
...
var require_parse_torrent = __commonJS({
  "node_modules/parse-torrent/index.js"(exports2, module2) {
    ...
    function parseTorrent2(torrentId) {
    ...
    function parseTorrentRemote(torrentId, opts, cb) {
      ...
      let parsedTorrent;
      try {
        parsedTorrent = parseTorrent2(torrentId);
      } catch (err) {
      }
      ...
      function parseOrThrow(torrentBuf) {
        try {
          parsedTorrent = parseTorrent2(torrentBuf);
        } catch (err) {
          return cb(err);
        }
        if (parsedTorrent && parsedTorrent.infoHash)
...
// qbittorrentAPI.js
var require_qbittorrentAPI = __commonJS({
  "qbittorrentAPI.js"(exports2, module2) {
    ...
    var qbittorrentAPI = class {
      ...
      async getTorrentList(options = {
        ...
        let data = [];
        if (options.filter)
          data.push("filter=" + options.filter);
        if (options.category)
          data.push("category=" + options.category);
        if (options.tag)
          data.push("tag=" + options.tag);
        if (options.sort)
          data.push("sort=" + options.sort);
        if (options.reverse)
          data.push("reverse=" + options.reverse);
        if (options.limit)
          data.push("limit=" + options.limit);
        if (options.offset)
          data.push("offset=" + options.offset);
        if (options.hashes?.length)
          data.push("hashes=" + options.hashes.join("|"));
        return await this.request("/torrents/info?" + encodeURI(data.join("&"))).then((res) => {
          if (res.status == 200) {
            return res.data;
          } else {
      ...
      async addTorrentFile(pathFile, saveDir, rename) {
        ...
        formData.append("torrents", fs2.createReadStream(pathFile));
        ...
        if (this.up_limit)
          formData.append("upLimit", this.up_limit);
        if (this.ratio)
          formData.append("ratioLimit", this.ratio);
        if (this.block_dl)
          formData.append("dlLimit", 1024);
        if (this.skip_check)
        ...
        return await this.request("/torrents/add", {
          ...
          if (res.status == 200) {
            if (this.include_trackers) {
              const _torrent = ParseTorrent(fs2.readFileSync(pathFile));
    ...
    module2.exports = qbittorrentAPI;
  }
});
...
var qt = require_qbittorrentAPI();
var parseTorrent = require_parse_torrent();
var path = require("path");
dotenv.configDotenv({
  path: path.join(__dirname, "stremio-seeds.config")
});
var CURRENT_OS = os.platform();
var LINUX_DEFAULT_CACHE_DIR = path.join(os.homedir(), "/.stremio-server/stremio-cache");
var WINDOWS_DEFAULT_CACHE_DIR = path.join(os.homedir(), "/AppData/Roaming/stremio/stremio-server/stremio-cache");
var MACOS_DEFAULT_CACHE_DIR = path.join(os.homedir(), "/Application Support/stremio-server/stremio-cache");
var CUSTOM_CACHE_DIR = process.env.CACHE_DIR;
var CacheDir;
if (CURRENT_OS === "win32")
...
var CUSTOM_CACHE_SIZE = process.env.CUSTOM_CACHE_SIZE;
var _CUSTOM_CACHE_SIZE;
if (CUSTOM_CACHE_SIZE) {
  let size = 0;
  if (CUSTOM_CACHE_SIZE?.match(/kb/i))
    size = parseFloat(CUSTOM_CACHE_SIZE) * 1024;
  else if (CUSTOM_CACHE_SIZE?.match(/mb/i))
    size = parseFloat(CUSTOM_CACHE_SIZE) * 1024 * 1024;
  else if (CUSTOM_CACHE_SIZE?.match(/gb/i))
    size = parseFloat(CUSTOM_CACHE_SIZE) * 1024 * 1024 * 1024;
  else if (CUSTOM_CACHE_SIZE?.match(/tb/i))
    size = parseFloat(CUSTOM_CACHE_SIZE) * 1024 * 1024 * 1024 * 1024;
  else
...
if (!process.env.QT_HOST)
  process.env.QT_HOST = "http://127.0.0.1";
if (!process.env.QT_PORT)
  process.env.QT_PORT = "6775";
var BASE_URL = process.env.QT_HOST + ":" + process.env.QT_PORT;
var USERNAME = process.env.USERNAME || "admin";
var PASSWORD = process.env.PASSWORD || "";
var UPLOAD_LIMIT = process.env.UPLOAD_LIMIT;
if (UPLOAD_LIMIT?.match(/kb/i))
  UPLOAD_LIMIT = parseInt(UPLOAD_LIMIT) * 1024;
else if (UPLOAD_LIMIT?.match(/mb/i))
  UPLOAD_LIMIT = parseInt(UPLOAD_LIMIT) * 1024 * 1024;
else if (UPLOAD_LIMIT?.match(/gb/i))
  UPLOAD_LIMIT = parseInt(UPLOAD_LIMIT) * 1024 * 1024 * 1024;
else
  UPLOAD_LIMIT = parseInt(UPLOAD_LIMIT);
var INTERVAL_CHECK = process.env.INTERVAL_CHECK;
if (INTERVAL_CHECK?.match(/sec/i))
  INTERVAL_CHECK = parseFloat(INTERVAL_CHECK) * 1e3;
else if (INTERVAL_CHECK?.match(/min/i))
  INTERVAL_CHECK = parseFloat(INTERVAL_CHECK) * 60 * 1e3;
else if (INTERVAL_CHECK?.match(/hour/i))
  INTERVAL_CHECK = parseFloat(INTERVAL_CHECK) * 60 * 60 * 1e3;
else if (INTERVAL_CHECK?.match(/day/i))
  INTERVAL_CHECK = parseFloat(INTERVAL_CHECK) * 24 * 60 * 60 * 1e3;
var START_SEED_PERCENT = process.env.START_SEED_PERCENT ? 0 < parseInt(process.env.START_SEED_PERCENT) <= 100 ? parseInt(process.env.START_SEED_PERCENT) : 90 : 90;
var RATIO_LIMIT = parseFloat(process.env.RATIO_LIMIT);
var BLOCK_DOWNLOAD = process.env.BLOCK_DOWNLOAD?.match(/true/i) ? true : false;
var SKIP_CHECKING = process.env.SKIP_CHECKING?.match(/true/i) ? true : false;
var INCLUDE_TRACKER = process.env.INCLUDE_STREMIO_TRACKER?.match(/true/i) ? true : false;
var KEEP_TORRENT_LOW_SEEDER = process.env.KEEP_TORRENT_LOW_SEEDER?.match(/true/i) ? true : false;
var CLEAN_CACHE_PERCENT = parseInt(process.env.CLEAN_CACHE_PERCENT) || 95;
var qbittorrent = new qt(BASE_URL, USERNAME, PASSWORD, { UPLOAD_LIMIT, RATIO_LIMIT, INCLUDE_TRACKER, BLOCK_DOWNLOAD, SKIP_CHECKING });
console.log("############### Stremio Seeds ##############");
console.log("OS:", CURRENT_OS);
console.log("Cache Dir:", CacheDir);
if (CUSTOM_CACHE_SIZE)
  console.log("Cache Size:", CUSTOM_CACHE_SIZE);
console.log("INTERVAL CHECK:", INTERVAL_CHECK);
console.log("RATIO LIMIT:", RATIO_LIMIT), console.log("UPLOAD LIMIT:", UPLOAD_LIMIT);
...
async function cleanTorrentsCache(currentSize) {
  ...
  }).sort((a, b) => b.time - a.time).slice(3);
  let _removed_size = 0;
...
function checkFolder(folderPath) {
  const bitfield = path.join(folderPath, "bitfield");
  const cacheTorrent = path.join(folderPath, "cache");
  if (!fs.existsSync(bitfield) || !fs.existsSync(cacheTorrent))
    return false;
  return true;
}
var createDirectories = (filePath) => {
...
async function addTorrent(folderPath) {
  ...
  if (process.env.FLATPAK_ID)
    _folderPath = folderPath.replace(os.homedir(), path.join(os.homedir(), "/.var/app/com.stremio.Stremio"));
